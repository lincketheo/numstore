...
typedef struct
{
  ...
  u64 *children;     // Pointer to child node ptrs
...
typedef struct
{
  ...
  char *key;        // The key
  u64 *data;        // The head of the data
} bmnode_kv;

static inline int
bmnode_valid (const bmnode *b)
{
  int ret = b != NULL;
  ret = ret && b->page;
  ret = ret && b->nkeys;
...
static inline int
bmnode_kv_valid (const bmnode_kv *b)
{
  return b && b->keylen && b->key && b->data && *b->keylen > 0;
}

DEFINE_DBG_ASSERT (bmnode_kv, bmnode_kv)

DEFINE_DBG_ASSERT (bmnode, bmnode)

static inline void
bmnode_init (bmnode *b)
{
  b->nkeys = (nkeys_t *)&b->page[0];
  b->children = (u64 *)(b->nkeys + 1);
  b->offsets = (offset_t *)(b->children + *b->nkeys + 1);
  bmnode_assert (b);
}

static inline bmnode_kv
bmnode_kv_create (u8 *head)
{
  bmnode_kv ret;
  ret.keylen = (keylen_t *)head;
  ret.key = (char *)(ret.keylen + 1);
  ret.data = (u64 *)(ret.key + *ret.keylen);
