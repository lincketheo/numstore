...
static inline float intervalf_center(const intervalf f) {
  return (f.vf - f.v0) / 2;
}

static inline bool intervalf_valid(const intervalf f) { return f.v0 <= f.vf; }

bool intervalf_f_in_exc(intervalf i, float v);

bool intervalf_if_in_exc(intervalf outer, intervalf inner);

static inline size_t intervals_len_inc_exc(const intervals s) {
  return s.s1 - s.s0;
}
...
static inline size_t finite_krange_size(finite_krange k) {
  return (size_t)abs(k.kf - k.k0);
}

static inline krange krange_default() {
  return (krange){
      .k0 = 0,
      .isinf = 1,
  };
...
ns_ret_t stack_push_grow(stack *s, void *src, size_t slen);
