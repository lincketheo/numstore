...
#ifndef NTEST
TEST (TT_UNIT, union_t_byte_size)
{
  struct union_t st;
  st.len = 4;
  st.keys = (struct string[]){
...
#ifndef NTEST
TEST (TT_UNIT, union_t_get_serial_size)
{
  struct union_t st;
  st.len = 4;
  st.keys = (struct string[]){
...
#ifndef NTEST
TEST (TT_UNIT, union_t_serialize)
{
  struct union_t st;
  st.len = 4;
  st.keys = (struct string[]){
...
#ifndef NTEST
TEST (TT_UNIT, union_t_deserialize_green_path)
{
  u8 data[] = { 0, 0,
                0, 0, 'f', 'o', 'o', (u8)T_PRIM, (u8)U32,
                0, 0, 'f', 'o', (u8)T_PRIM, (u8)U8,
...
#ifndef NTEST
TEST (TT_UNIT, union_t_deserialize_red_path)
{
  u8 data[] = { 0, 0,
                0, 0, 'f', 'o', 'o', (u8)T_PRIM, (u8)U32,
...
err_t
union_t_random (struct union_t *un, struct lalloc *alloc, u32 depth, error *e)
{
  ASSERT (un);

  un->len = (u16)randu32r (1, 5);

  un->keys = (struct string *)lmalloc (alloc, un->len, sizeof (struct string), e);
  if (!un->keys)
    {
      return e->cause_code;
    }

  un->types = (struct type *)lmalloc (alloc, un->len, sizeof (struct type), e);
  if (!un->types)
    {
      return e->cause_code;
    }
