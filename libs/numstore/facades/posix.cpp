...
ns_ret_t open_f(int *dest, const char *path, const int flags, ...) {
  ASSERT(dest);
  ASSERT(path);
...
ns_ret_t read_f(size_t *read_out, int fd, void *buf, size_t bytes) {
  ASSERT(read_out);
  ASSERT(fd > 0);
  ASSERT(buf);
  ASSERT(bytes > 0);
...
PRIVATE FILE *fifoopen(const char *path, const char *mode) {
  if (mkfifo(path, 0666))
    return NULL;
...
ns_ret_t fifoopen_f(FILE **dest, const char *path, const char *modes) {
  ASSERT(dest);
  ASSERT(path);
  ASSERT(modes);
