...
extern "C" {
#include <sys/stat.h>
PRIVATE ns_ret_t fsize_seekable(size_t *dest, FILE *fp) {
  ASSERT(fp);
  size_t start, bytes;
...
PRIVATE ns_ret_t fsize_non_seekable(size_t *dest, FILE *fp) {
  ASSERT(fp);
  size_t bytes = 0;
...
ns_ret_t fsize(size_t *dest, FILE *fp, const size_t size) {
  size_t blen;
