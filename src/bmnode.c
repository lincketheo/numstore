...
/////////////////////////////// HELPERS
static inline offset_t bmnode_prefix_tail(bmnode* b)
{
...
static inline offset_t bmnode_min_offset(bmnode* b)
{
  bmnode_assert(b);
...
static inline void unsafe_bmnode_expand_and_split(bmnode* b, u32 idx)
{
  bmnode_assert(b);
  assert(idx <= *b->nkeys);
...
static inline void unsafe_bmnode_expand_one(bmnode* b)
{
  bmnode_assert(b);
    ...
    page_ptr data)
{
  ...
  size_t kv_size = sizeof(keylen_t) + (size_t)keylen + sizeof(page_ptr);
  size_t toadd = sizeof(page_ptr) + sizeof(offset_t) + kv_size;
