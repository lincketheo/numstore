...
usize srange_copy(u8* dest, usize dnelem, const u8* src, usize snelem, srange range,
    usize size)
{
  assert(dest);
  assert(src);
...
TEST(srange_copy)
{
  ...
  usize copied = srange_copy((u8*)dest, 10, (u8*)src, 20, r, sizeof(u32));
  ...
  copied = srange_copy((u8*)dest, 0, (u8*)empty_src, 0, r7, sizeof(u32));
  test_assert_equal(copied, 0lu, "%zu");
