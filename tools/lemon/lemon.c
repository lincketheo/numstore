...
static void *
lemon_calloc (size_t nElem, size_t sz)
{
...
static void *
lemon_realloc (void *pOld, size_t nNew)
{
  void *pNew;
  MemChunk *p;
  if (pOld == 0)
    return lemon_malloc (nNew);
  p = (MemChunk *)pOld;
  p--;
  if (p->sz >= nNew)
    return pOld;
...
static int
lemon_vsprintf (char *str, const char *zFormat, va_list ap)
{
  int i, j, k, c;
  int nUsed = 0;
  const char *z;
...
static int
lemon_sprintf (char *str, const char *format, ...)
{
  va_list ap;
  int rc;
  va_start (ap, format);
...
static void
lemon_strcpy (char *dest, const char *src)
{
  while ((*(dest++) = *(src++)) != 0)
    {
    }
...
static void
lemon_strcat (char *dest, const char *src)
{
  while (*dest)
    dest++;
...
acttab *
acttab_alloc (int nsymbol, int nterminal)
{
...
*/
void
acttab_action (acttab *p, int lookahead, int action)
{
...
*/
int
acttab_insert (acttab *p, int makeItSafe)
{
  int i, j, k, n, end;
...
PRIVATE void
deleteconfig (struct config *old)
{
  old->next = freelist;
  freelist = old;
}
...
struct config *
Configlist_addbasis (struct rule *rp, int dot)
{
  struct config *cfp, model;
...
void
Configlist_closure (struct lemon *lemp)
{
...
void
Configlist_eat (struct config *cfp)
{
...
static struct rule *
Rule_merge (struct rule *pA, struct rule *pB)
{
 ...
 */
static void
stats_line (const char *zLabel, int iValue)
{
...
*/
static int
defineCmp (const void *pA, const void *pB)
{
...
int
OptInit (char **a, struct s_options *o, FILE *err)
{
  int errcnt = 0;
  g_argv = a;
  op = o;
  ...
  switch (psp->state)
    ...
    case IN_RHS:
      if (x[0] == '.')
        {
          struct rule *rp;
          rp = (struct rule *)lemon_calloc (sizeof (struct rule) + sizeof (struct symbol *) * psp->nrhs + sizeof (char *) * psp->nrhs, 1);
          if (rp == 0)
            {
              ErrorMsg (psp->filename, psp->tokenlineno,
...
*/
static int
eval_preprocessor_boolean (char *z, int lineno)
{
  int neg = 0;
  int res = 0;
  int okTerm = 1;
 ...
 */
void
rule_print (FILE *out, struct rule *rp)
{
  int i, j;
 ...
 */
void
RulePrint (FILE *fp, struct rule *rp, int iCursor)
{
  struct symbol *sp;
  int i, j;
 ...
 */
PRIVATE void
tplt_skip_header (FILE *in, int *lineno)
{
  char line[LINESIZE];
  while (fgets (line, LINESIZE, in) && (line[0] != '%' || line[1] != '%'))
    {
...
*/
static const char *
minimum_size_type (int lwr, int upr, int *pnByte)
{
...
Symbolcmpp (const void *_a, const void *_b)
{
  ...
  int i1 = a->type == MULTITERMINAL ? 3 : a->name[0] > 'Z' ? 2
                                                           : 1;
  int i2 = b->type == MULTITERMINAL ? 3 : b->name[0] > 'Z' ? 2
                                                           : 1;
...
int
Configcmp (const char *_a, const char *_b)
{
...
/* Compare two states */
PRIVATE int
statecmp (struct config *a, struct config *b)
{
  int rc;
